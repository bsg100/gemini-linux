...
| 7 | Power management | **Charger identity corrected 2026-07-14** (research.md section 8): TI BQ25896 at i2c0 0x6b, not RT9466. Mainline `bq25890_charger.c` in use; see hardware.md Battery Charger row. Fuel gauge (MT6351) has no mainline support - charger-only + userspace voltage monitor remains the Phase 7 minimum. |
| 8 | Networking | **B-22 RESOLVED 2026-07-16 - simultaneous left-port charging + right-port USB ethernet confirmed on hardware (build #255):** right port (usb1/MUSB, `dr_mode="host"`) runs an ethernet adapter while the left port (mtu3, `dr_mode="peripheral"`) charges via BQ25896 and auto-enumerates its RNDIS gadget. USB host-mode history (B-19/B-20), the WiFi-vs-USB decision, and the CONSYS spike (B-21, parked at Gate G2b) are all recorded in blockers.md - do not duplicate that history here. |
